...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - Recupera le ultime notizie in tutto il mondo dall'Italia"
        ...
        openGraph={{
          ...
          title:
            "Top News Feed Live - Recupera le ultime notizie in tutto il mondo dall'Italia",
          description:
            "Questo portale ti consente di ottenere le migliori notizie in Italia e nel mondo online insieme a titoli e immagini, puoi anche cercare notizie in base al paese in diverse lingue, fornendo risultati di ricerca migliori rispetto ad altri portali.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Italy Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/it/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=it&pageSize=5&page=${pageNumber}`,
    {
      headers: {
