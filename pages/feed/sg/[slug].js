...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - Fetch latest news across the globe from Singapore"
        ...
        openGraph={{
          ...
          title:
            "Top News Feed Live - Fetch latest news across the globe from Singapore",
          description:
            "This portal allows you to get top Singapore and worldwide news online along with titles and images, you can also search news country wise in different languages, providing better search results than other portals.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Singapore Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/sg/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=sg&pageSize=5&page=${pageNumber}`,
    {
      headers: {
