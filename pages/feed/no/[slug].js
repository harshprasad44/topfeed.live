...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - Hent siste nytt over hele verden fra Norge"
        ...
        openGraph={{
          ...
          title:
            "Top News Feed Live - Hent siste nytt over hele verden fra Norge",
          description:
            "Denne portalen lar deg få topp Norge og verdensomspennende nyheter på nettet sammen med titler og bilder. Du kan også søke nyheter landsmessig på forskjellige språk, noe som gir bedre søkeresultater enn andre portaler.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Norway Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/no/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=no&pageSize=5&page=${pageNumber}`,
    {
      headers: {
