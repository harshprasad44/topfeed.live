...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - Récupérez les dernières nouvelles à travers le monde depuis la Belgique"
        ...
        openGraph={{
          ...
          title:
            "Top News Feed Live - Récupérez les dernières nouvelles à travers le monde depuis la Belgique",
          description:
            "Ce portail vous permet d'obtenir en ligne les principales actualités de la Belgique et du monde entier, ainsi que des titres et des images, vous pouvez également rechercher des actualités par pays dans différentes langues, offrant de meilleurs résultats de recherche que d'autres portails.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Belgium Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/be/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=be&pageSize=5&page=${pageNumber}`,
    {
      headers: {
