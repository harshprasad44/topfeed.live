...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Najlepšie správy naživo - načítajte zo Slovenska najnovšie správy z celého sveta"
        ...
        openGraph={{
          ...
          title:
            "Najlepšie správy naživo - načítajte zo Slovenska najnovšie správy z celého sveta",
          description:
            "Tento portál vám umožňuje získať top slovenské a svetové správy online spolu s titulmi a obrázkami. Môžete tiež vyhľadávať spravodajské informácie v rôznych jazykoch a poskytovať lepšie výsledky vyhľadávania ako iné portály.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Slovakia Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/sk/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=sk&pageSize=5&page=${pageNumber}`,
    {
      headers: {
