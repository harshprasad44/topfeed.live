...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Najpopularnije vesti uživo - Preuzmite najnovije vesti iz celog sveta iz Srbije"
        ...
        openGraph={{
          ...
          title:
            "Najpopularnije vesti uživo - Preuzmite najnovije vesti iz celog sveta iz Srbije",
          description:
            "Ovaj portal vam omogućava da putem Interneta dobijete najnovije vesti iz Srbije i sveta, zajedno sa naslovima i slikama, takođe možete pametno pretraživati ​​vesti na različitim jezicima, pružajući bolje rezultate pretraživanja od ostalih portala.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Serbia Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/rs/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=rs&pageSize=5&page=${pageNumber}`,
    {
      headers: {
