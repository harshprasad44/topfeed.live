...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - احصل على آخر الأخبار عبر العالم من مصر"
        ...
        openGraph={{
          ...
          title: "Top News Feed Live - احصل على آخر الأخبار عبر العالم من مصر",
          description:
            "تتيح لك هذه البوابة الحصول على أفضل أخبار مصر والعالم عبر الإنترنت إلى جانب العناوين والصور ، كما يمكنك البحث عن بلد الأخبار بلغات مختلفة ، مما يوفر نتائج بحث أفضل من البوابات الأخرى.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Egypt Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/eg/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=eg&pageSize=5&page=${pageNumber}`,
    {
      headers: {
