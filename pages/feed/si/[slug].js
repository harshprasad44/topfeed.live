...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - Poiščite najnovejše novice po vsem svetu iz Slovenije"
        ...
        openGraph={{
          ...
          title:
            "Top News Feed Live - Poiščite najnovejše novice po vsem svetu iz Slovenije",
          description:
            "Ta portal vam omogoča, da na spletu po naslovih in slikah poiščete najboljše slovenske in svetovne novice, v različnih jezikih pa lahko poiščete tudi novice po državah, ki zagotavljajo boljše rezultate iskanja kot drugi portali.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>Slovenia Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/si/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=si&pageSize=5&page=${pageNumber}`,
    {
      headers: {
