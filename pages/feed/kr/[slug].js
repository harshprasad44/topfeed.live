...
const Feed = ({ pageNumber, articles }) => {
  ...
  return (
    <>
      <NextSeo
        title="Top News Feed Live - 한국의 전 세계 최신 뉴스를 가져옵니다."
        ...
        openGraph={{
          ...
          title: "Top News Feed Live - 한국의 전 세계 최신 뉴스를 가져옵니다.",
          description:
            "이 포털을 통해 제목과 이미지와 함께 한국 및 세계 최고의 뉴스를 온라인으로 볼 수 있으며, 다른 언어로 뉴스 국가를 현명하게 검색 할 수도있어 다른 포털보다 더 나은 검색 결과를 제공합니다.",
          images: [
            {
      ...
      <div className="page-container">
        <Toolbar />
        <Country />
        <h3 className={styles.countryFeed}>South Korea Feed</h3>
        ...
        <div className={styles.paginator}>
          <div
            onClick={() => {
              if (pageNumber > 1) {
                router.push(`/feed/kr/${pageNumber - 1}`);
              }
            }}
...
export const getServerSideProps = async (pageContext) => {
  ...
  const apiResponse = await fetch(
    `https://newsapi.org/v2/top-headlines?country=kr&pageSize=5&page=${pageNumber}`,
    {
      headers: {
